...
void send_network_configuration_html()
{
	...
	if (MyWebServer.args() > 0 )  // Save Settings
	{
		...
		for ( uint8_t i = 0; i < MyWebServer.args(); i++ ) {
			if (MyWebServer.argName(i) == "ssid") config.ssid =   urldecode(MyWebServer.arg(i));
			if (MyWebServer.argName(i) == "WiFiKey") config.WiFiKey =    urldecode(MyWebServer.arg(i));
		 ...
		 MyWebServer.send_P ( 200, "text/html", PAGE_WaitAndReload );
		storage_write();
		ConfigureWifi();
		storage_read();
	}
	...
	{
		MyWebServer.send_P ( 200, "text/html", PAGE_NetworkConfiguration ); 
	}
