...
unsigned int BL0937::getActivePower() {
    ...
    _power = (_power_pulse_width > 0) ? ((_power_multiplier * 1) / (_power_pulse_width / 2)) : 0;
    return _power;
}


unsigned long BL0937::getActivePowerWidth() {
    return _power_pulse_width;
}

unsigned long BL0937::getActivePowerCount() {
    return _pulse_count;
}
...
void ICACHE_RAM_ATTR BL0937::cf_interrupt() {
    ...
    if (_power_pulse_width <= _pulse_timeout) _pulse_count++;
}
...
void ICACHE_RAM_ATTR BL0937::cf1_interrupt() {
    ...
    if (_first_cf1_interrupt) {
        _first_cf1_interrupt = false;
    }
    else {
        CF1_pulse_width = now - _last_cf1_interrupt;
        if (_mode == _current_mode) {
                _current_pulse_width = CF1_pulse_width;
        }
        else {
                _voltage_pulse_width = CF1_pulse_width;
        }
        _mode = 1 - _mode;
        digitalWrite(_sel_pin, _mode);
        _first_cf1_interrupt = true;
     }
